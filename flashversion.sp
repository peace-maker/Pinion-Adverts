...
{
	RegConsoleCmd("sm_flashversion", Cmd_FlashVersion, "Shows your flash player version.");
...
public Action:Cmd_FlashVersion(client, args)
{
	...
	Flash_GetClientVersion(iTarget, false, Flash_OnGetClientVersion, client);
	
	return Plugin_Handled;
}

public Flash_OnGetClientVersion(client, flashversion[4], bool:bError, any:data)
{
	if(!data)
	{
	...
	{
		PrintToChat(data, "%s%N's flash player version: %d.%d.%d.%d", (bError?"Failed to get ":""), client, flashversion[0], flashversion[1], flashversion[2], flashversion[3]);
		PrintToConsole(data, "%s%N's flash player version: %d.%d.%d.%d", (bError?"Failed to get ":""), client, flashversion[0], flashversion[1], flashversion[2], flashversion[3]);
	}
}
